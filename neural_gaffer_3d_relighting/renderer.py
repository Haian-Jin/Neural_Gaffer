...
def evaluation(test_dataset,tensorf, args, renderer, savePath=None, N_vis=5, prtx='', N_samples=-1,
    ...
    imageio.mimsave(f'{savePath}/{prtx}video.mp4', np.stack(rgb_maps), fps=25, quality=9, codec='libx264')
    imageio.mimsave(f'{savePath}/{prtx}depthvideo.mp4', np.stack(depth_maps), fps=25, quality=9, codec='libx264')
...
def evaluation_path(test_dataset,tensorf, c2ws, renderer, savePath=None, N_vis=5, prtx='', N_samples=-1,
                    white_bg=False, ndc_ray=False, compute_extra_metrics=True, device='cuda', color_tf=None):
    PSNRs, rgb_maps, depth_maps = [], [], []
    ssims,l_alex,l_vgg=[],[],[]
    os.makedirs(savePath, exist_ok=True)
    os.makedirs(savePath+"/rgbd", exist_ok=True)
